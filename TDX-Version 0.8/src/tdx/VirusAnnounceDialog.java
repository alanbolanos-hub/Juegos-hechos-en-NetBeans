 ...
 *
 * Características:
 *  • Personaje "Dr. Byte" con animación de 3 estados (idle / habla / señala).
 *  • Cuadro de diálogo con texto que aparece letra a letra (typewriter).
 *  • Panel lateral con: forma del virus, datos clínicos, tratamiento.
 *  • Navegación entre múltiples virus con botón "SIGUIENTE →".
 *  • Fondo oscuro con overlay semitransparente (simula pausa de juego).
 *
 *  ══ INTEGRACIÓN ═══════════════════════════════════════════════════════
 *  Llama en tu bucle de juego cuando wave == adminCfg.virusAnnounceWave:
 *
...
public class VirusAnnounceDialog extends JDialog {
    ...
    private void startTypewriter(String text) {
        ...
        typewriterTimer = new Timer(32, e -> {
            if (typewriterPos < fullText.length()) {
                ...
                if (typewriterPos % 3 == 0) characterPanel.repaint();
            } else {
    ...
    private void startAnimLoop() {
        gameLoopTimer = new Timer(80, e -> {
            ...
            // parpadeo idle
            blinkCounter++;
            if (blinkCounter > 18) blinkCounter = 0;
    ...
    private class VirusShapePanel extends JPanel {
        ...
        VirusShapePanel(VirusData v) {
            ...
            setOpaque(false);
            // Pulso suave
            pulseTimer = new Timer(50, e -> { pulseT += 0.08f; repaint(); });
            pulseTimer.start();
            addHierarchyListener(e -> {
                if (!isDisplayable() && pulseTimer != null) pulseTimer.stop();
            });
        }
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            switch (v.shape) {
                case SPHERE:       drawSphereVirus(g2, cx, cy, pulse);     break;
                case ICOSAHEDRON:  drawIcosaVirus(g2, cx, cy, pulse);      break;
                case ELONGATED:    drawElongatedVirus(g2, cx, cy, pulse);  break;
                case BACTERIUM:    drawBacteriumVirus(g2, cx, cy, pulse);  break;
                case CORONA:       drawCoronaVirus(g2, cx, cy, pulse);     break;
            }
        ...
        private void drawBacteriumVirus(Graphics2D g2, int cx, int cy, float pulse) {
            ...
            for (int i = 0; i < 40; i++) {
                ...
                if (i == 0) flagelo.moveTo(fx, fy);
                else flagelo.lineTo(fx, fy);
            }
            g2.draw(flagelo);
            // Núcleo
    ...
    // DATOS DE VIRUS
    // ─────────────────────────────────────────────────────────────────────
    enum VirusShape { SPHERE, ICOSAHEDRON, ELONGATED, BACTERIUM, CORONA }
    ...
    static class VirusData {
        ...
        VirusData(String name, String sci, String type, VirusShape shape, Color color,
            ...
            this.name = name; this.scientificName = sci; this.type = type;
            this.shape = shape; this.color = color; this.danger = danger;
            this.target = target; this.symptoms = symptoms; this.treatment = treatment;
        }

        Color dangerColor() {
            if (danger >= 5) return new Color(220, 40, 40);
            if (danger >= 4) return new Color(220, 120, 40);
            if (danger >= 3) return new Color(220, 200, 40);
            if (danger >= 2) return new Color(120, 200, 40);
